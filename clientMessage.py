...
class Message:
    ...
    def _process_response_binary_content(self):
        content = self.response
        unpacked = struct.unpack(">27s6si", content)
        decoded = unpacked[1].decode("utf-8")
        if "result" in decoded:
            print()
            print(content)
            print("Result:", unpacked[2])
        else:
    ...
    def close(self):
        ...
        logger.info("Closed connection to %s on port %s", self.addr[0], self.addr[1])
        try:
